...
(function (factory) {
	...
	var quadtree = L.Class.extend({
		...
		add: function (elem) {
			...
			if (
				self.options.maxObjectsPerLevel < self.elements.length &&
		...
		updateChildNodesBounds: function () {
			...
			if (0 != self.nodes.length) {
				...
				self.nodes[0].bounds = L.latLngBounds(
					L.latLng(center.lat, nwChild.lng),
					L.latLng(nwChild.lat, center.lng)
				);
				self.nodes[0].updateChildNodesBounds();
				//Update the top-right quadrant
				self.nodes[1].bounds = L.latLngBounds(
				...
				self.nodes[2].bounds = L.latLngBounds(
					L.latLng(seChild.lat, nwChild.lng),
					L.latLng(center.lat, center.lng)
				);
				self.nodes[2].updateChildNodesBounds();
				//Update the bottom-right quadrant
				self.nodes[3].bounds = L.latLngBounds(
					L.latLng(seChild.lat, center.lng),
		...
		updateChildNodesElements: function () {
			...
			if (0 != self.nodes.length) {
				...
				if (
					self.options.maxObjectsPerLevel < elements.length &&
					self.depth < self.options.maxLevels
				) {
					self.splitNode();
				...
				if (!intersects) {
					...
					for (var i = 0, len = self.nodes.length; i < len; ++i) {
						colliders = colliders.concat(
							self.nodes[i].getPossibleColliders(bounds)
						);
					}
				}
			}
